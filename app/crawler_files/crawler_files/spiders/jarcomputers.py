...
class JarcomputersSpider(scrapy.Spider):
    ...
    def extractInfo(self, response):
        ...
        item = {
            "productName": response.xpath(
                """/html/body/div[3]/div[3]/div[2]/div[1]/div[2]/div[2]/div[1]/h1/text()"""
            )
            .get()
            .split(",")[0],
            "productNumber": response.xpath(
                """/html/body/div[3]/div[3]/div[2]/div[1]/div[2]/div[2]/div[2]/ul[1]/li[4]/b/text()"""
            ).get(),
            "urlLink": response.url,
            "productStore": "jarcomputers",
            "imgForProductLink": response.xpath(
                """//*[@id="main_image"]/a/@href"""
            ).get(),
        }

        isProductAvailable = response.xpath(
            """/html/body/div[3]/div[3]/div[2]/div[1]/div[2]/div[2]/div[2]/ul[1]/li[6]/span/text()"""
        ).get()

        # Check if the product is not avaible
        if (
            isProductAvailable == "Няма наличност"
            or isProductAvailable == "Стар продукт"
            or isProductAvailable == "Не е наличен"
        ):
            item["isProductAvailable"] = False
            item["productPrice"] = None
        else:
            item["isProductAvailable"] = True
            item["productPrice"] = float(
                response.xpath("""//*[@class="price"]/text()""").get()
                + response.xpath("""//*[@class="price2"]/text()""").get()
            )
    ...
    def parse(self, response):
        ...
        self.href = response.xpath(
            """/html/body/div[3]/div[3]/div[2]/div/div[4]/ol/li/div[2]/p/span[2]/a/@href"""
        ).extract()
