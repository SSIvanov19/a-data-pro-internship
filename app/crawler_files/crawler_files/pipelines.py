...
class CrawlerFilesPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.error(err, extra={"tags": {"service": "MSSQL"}})

        self.cursor = self.conn.cursor()

    def process_item(self, item, spider):
        # Check if the item has a product number
        if item["productNumber"] is None:
            self.logger.warning(
                'Product Number on product with name: "{}" is None/Null'.format(
                    item["productName"]
                ),
        ...
        self.logger.info(
            "Item stored: {}".format(item["productName"]),
